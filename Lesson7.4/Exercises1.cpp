...
public:
	...
	// update node trong heap
	bool update(T oldNode, T newNode) {
		int index = findNodeIndex(oldNode);
		if (index >= 0) {
			data[index] = newNode;
			int parentIndex = (index - 1) / 2;
...
int main() {
	...
	do
	{
		...
		cout << "4. Cap nhat node co gia tri x.\n";
		cout << "5. Xoa node co gia tri x.\n";
		cout << "6. Cho biet phan tu lon nhat trong heap.\n";
		cout << "7. Cho biet kich thuoc cua heap.\n";
		...
		case 4:
			if (heap.isEmpty()) {
				...
				int x, newX;
				cout << "Nhap gia tri can update x = ";
				cin >> x;
				cout << "Gia tri moi cua x = ";
				cin >> newX;
				if (heap.update(x, newX)) {
					cout << "Cap nhat thanh cong thanh cong.\n";
				}
				else {
					cout << "Xoa that bai.\n";
				}
